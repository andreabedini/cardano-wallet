...
import Cardano.Wallet
    ( BalancingResultInfo (BalancingResultInfo)
    , ErrBalanceTx (..)
...
balanceTransactionSpec = describe "balanceTransaction" $ do
    ...
    describe "change address generation" $ do
        ...
        let expectedChange = flip evalState s0
                $ replicateM nChange
                $ state @Identity (genChange @DummyChangeState ())
        ...
        let (tx, BalancingResultInfo change s') =
        ...
        it "returns info about used addresses in form of [Address] and s'" $ do
    ...
    describe "effect of txMaxSize on coin selection" $ do
        ...
        let balance = fmap fst . balanceTransaction' wallet testStdGenSeed
        let totalOutput tx =
                let (wtx, _, _, _, _, _) =
  ...
  where
    ...
    balanceTx tx = flip evalRand (stdGenFromSeed testStdGenSeed) $ runExceptT $ fst <$>
...
balanceTransaction'
    ...
    -> Either
        ErrBalanceTx
        (Cardano.Tx era, BalancingResultInfo (SeqState 'Mainnet ShelleyKey))
...
instance GenChange DummyChangeState where
    type ArgGenChange DummyChangeState = ()
    genChange () (DummyChangeState i) =
...
balanceTransactionWithDummyChangeState
    ...
    -> Either
        ErrBalanceTx
        (Cardano.Tx era, BalancingResultInfo DummyChangeState)
...
prop_balanceTransactionValid wallet (ShowBuildable partialTx) seed
    = withMaxSuccess 1_000 $ do
        ...
        let res = fst <$> balanceTransaction'
                wallet
                seed
                partialTx
        case res of
